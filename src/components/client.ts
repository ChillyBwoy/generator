...
export class <%= className %> {
  ...
  private request(methodName: string, params: any[]): Promise<any> {
    const methodObject = <%= className %>.openrpcDocument.methods.find(({name}) => name === methodName) as MethodObject;
    ...
    if (methodObject.paramStructure && methodObject.paramStructure === "by-name") {
      const paramNames = methodObject.params.map((param) => (param as ContentDescriptorObject).name);
      rpcParams = _zipObject(params, paramNames);
    } else {
      rpcParams = params;
    }
    if (notification) {
